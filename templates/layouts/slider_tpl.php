<?php
    ...
    $slider=$db->rawQuery("select ten_$lang,photo_$lang photo,link from #_photo where hienthi=1 and type=? order by stt asc,id desc",array('slider-tpl'));
...
<section id="slider-top" class="clearfix p-relative">
                   ...
                   <div data-u="slides"  style="cursor:default;position:relative;top:0px;left:0px;width:1366px;height:342px;overflow:hidden;">
                        <?php foreach ($slider as $k => $v){ ?>
                            <div data-b="0" data-idle="2000">
                                <a href="<?=$v['link']?>" title="<?=$v['ten']?>"><img width="1440" height="342" style="width: 100%;aspect-ratio: 1440 / 342;object-fit: cover;height: auto;" data-u="image" src="<?=_upload_hinhanh_l.$v['photo']?>" alt="<?=$v['ten_'.$lang]?>" <?=$func->errorImg()?>/></a>
                            </div>
                        <?php }?>
                    </div>
    ...
    <div class="slider-top__header-search">

        <?php include _layouts."searchDes.php";?>
