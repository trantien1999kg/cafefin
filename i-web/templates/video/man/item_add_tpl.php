            ...
            <ul id="breadcrumbs" class="breadcrumbs">
                <li><a
                        href="index.html?com=video&act=add<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span><?=($_GET['act']=='edit') ? _sua : _them?>
                            <?=$GLOBAL['video'][$_GET["type"]]['title_main']?></span></a></li>
        ...
        <div class="mtop0">
            ...
            <div class="<?=($config_cap1['full']==true) ? 'oneOne':'colLeft' ?>">
                <div class="widget mtop0">
                    ...
                    <?php  if($GLOBAL['video'][$_GET["type"]]['ten2']==true){ ?>
                        <div class="formRow lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                        <label><?=_tieude?> 2 [<?=$v?>]</label>
                        <div class="formRight">
                            <input type="text" name="data[ten2_<?=$k?>]" title="Nhập tên 2 danh mục" id="ten2_<?=$k?>"
                                class="tipS validate[required]" value="<?=@$item['ten2_'.$k]?>" />
                        </div>
                        <div class="clear"></div>
                    </div>
                    <?php }?>
                    <?php if($GLOBAL['mota']==true){ ?>
                    <div class="formRow lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                        <label><?=_mota?> [<?=$v?>]</label>
                        <div class="ck_editor">
                            <textarea title="Nhập mô tả . " data-height="400" id="mota_<?=$k?>"
                                <?= ($GLOBAL['mota-ckeditor']==true) ? 'class="ck_editors"':'rows="8"' ?>
            ...
            <div class="<?=($config_cap1['full']==true) ? 'oneOne':'colRight' ?>">
                <div class="widget mtop0">
                    <div class="title"><img src="./images/icons/dark/record.png" alt="" class="titleIcon" />
                        <h6><?=_thongtin?> <?=$GLOBAL['video'][$_GET["type"]]['title_main']?></h6>
                    </div>
                    <?php if($GLOBAL['video'][$_GET["type"]]['img']==true){ ?>
                    <div class="formRow ">
                        ...
                        <div class="formRight">
                            ...
                            <span style="display: inline-block; line-height: 30px;color:#CCC;">
                                Width:
                                <?=$GLOBAL['video'][$_GET["type"]]['img-width']*$GLOBAL[$_GET["type"]]['video']['img-ratio']?>px -
                                Height:
                                <?=$GLOBAL['video'][$_GET["type"]]['img-height']*$GLOBAL['video'][$_GET["type"]]['img-ratio']?>px
                            </span>
                        </div>
                        <div class="clear"></div>
                    </div>
                    ...
                    <?php if($GLOBAL['video'][$_GET["type"]]['upload']==true){ ?>
                    <div class="formRow ">
                    ...
                    <?php if($GLOBAL['video'][$_GET["type"]]['link']==true){ ?>
                    <?php if($_GET['act']=='edit'){?>
                    <div class="formRow">
                    ...
                    <?php if($GLOBAL['video'][$_GET["type"]]['link1']==true){ ?>
                    
                    <div class="formRow">
                        <label>Link kèm 1</label>
                        <div class="formRight">
                            <input type="text" name="data[link1]" title="Nhập tên link1" id="ten_en"
                                class="tipS validate[required]" value="<?=@$item['link1']?>" />
                        </div>
                        <div class="clear"></div>
                    </div>
                    <?php } ?>

                    <?php if($GLOBAL['video'][$_GET["type"]]['link2']==true){ ?>
                    
                    <div class="formRow">
                        <label>Link kèm 2</label>
                        <div class="formRight">
                            <input type="text" name="data[link2]" title="Nhập tên link2" id="ten_en"
                                class="tipS validate[required]" value="<?=@$item['link2']?>" />
                        </div>
                        <div class="clear"></div>
                    </div>
                    <?php } ?>
                    ...
                    <?php if($GLOBAL['video'][$_GET["type"]]['seo']==true){ ?>
