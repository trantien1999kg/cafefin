<?php
    ...
    if($BOOKING==true){
        ...
        $sql = "SELECT COUNT(*) as numb FROM #_booking where type=? ";
        $row_booking = $db->rawQueryOne($sql,array('client'));

        $sqlgoilai = "SELECT COUNT(*) as numb FROM #_booking where type=? ";
        $row_goilai = $db->rawQueryOne($sqlgoilai,array('goi-lai'));

    }
    
    $tong_count = $row_lienhe['numb']+$row_giohang['numb']+$row_dangky['numb']+$row_booking['numb']+$row_goilai["numb"];
                ...
                <li class="ddnew" style="position:relative">
                    ...
                    <div class="popover-notifications" style="top: 35px;right: 0px;transform-origin: 60px top;">
                        <div>
                            ...
                            <?php if($BOOKING==true){?>
                            <div>
                                <a href="index.html?com=booking&act=man&type=client" title="" ><span>Đăng ký nhận tin</span> <span>(<?=($row_booking['numb']==0) ? 0 : $row_booking['numb']?>)</span> 
                                    </a>
                            </div>
                            <?php }?>
                            <?php if($BOOKING==true){?>
                            <div>
                                <a href="index.html?com=booking&act=man&type=goi-lai" title="" ><span>Yêu cầu gọi lại</span> <span>(<?=($row_goilai['numb']==0) ? 0 : $row_goilai['numb']?>)</span> 
                                    </a>
                            </div>
                            <?php }?>
                        </div>
