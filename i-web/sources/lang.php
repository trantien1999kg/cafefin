...
function get_langs(){
	...
	if($_REQUEST['keyword']!=''){
		...
		$where.=" where item like '%$ten%' or lang_vi like '%$ten%' or lang_en like '%$ten%' or lang_jp or lang_cn like '%$ten%'";
...
function create_lang(){
	...
	$file_langcn = "../sources/langWeb/lang_cn.php";
	if (file_exists ($file_langcn)){
		unlink($file_langcn);
		$file_langcn = @fopen ($file_langcn, "w+") or die ("Couldn't create the file");
		fwrite($file_langcn, "<?php \n");
		foreach ($items as $key => $value) {
			$line = "@define ('".$value['item']."','".$value['lang_cn']."');\n";
			fwrite($file_langcn, $line);
		}
		fwrite($file_langcn, '?>');
		fclose ($file_langcn);
	}
