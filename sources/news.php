<?php  if(!defined('_source')) die("Error");
    ...
    if($com=='dich-vu'){
        ...
        $per_page = $row_setting['page_dichvu'];

    }

    if($com=='bao-gia'){

        $per_page = $row_setting['page_baogia'];
    ...
    if(!empty($idd)){
        ...
        $row_list=$db->rawQueryOne("select id,ten_$lang,tenkhongdau_$lang as tenkhongdau,mota_$lang,noidung_$lang,options,mucluc,photo,type from #_baiviet where hienthi=1 and type=? and id=? order by stt asc, id desc",array('dia-diem',$idd));
        ...
        if($row_list){
            ...
            $subWhere.=" and id_diadiem={$row_list['id']}";
            ...
            $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau from #_baiviet where hienthi=1 and id_diadiem=? and type=? $order_by limit $startpoint,$per_page",array($row_list['id'],'bat-dong-san'));


            $count=$db->rawQueryOne("select COUNT(*) as `numb` from #_baiviet where hienthi=1 and id_diadiem=? and type=?", array($row_list['id'],'bat-dong-san'));
    ...
    if(empty($id) && empty($idl) && empty($idc) && empty($idi) && empty($ids)){
        ...
        $str_breadcrumbs = $breadcrumbs->getUrl('trang chủ',array(array('alias'=>$func->getCom($type),'name'=>$title_seo)));
        ...
        // if(count($row_list)>0 && $type='dat-phong'){


        //     $template='bookroom/bookroom_list';
        ...
        $post_tags = $db->rawQuery("select id,ten_$lang as ten,tenkhongdau_$lang as tenkhongdau,type from #_tags where hienthi=1 and id in ($im_tag) and type=? order by stt asc",array('tags-dich-vu'));
