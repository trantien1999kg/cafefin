<?php  if(!defined('_source')) die("Error");
	...
	if($id!=''){
		...
		$where = " #_video where type='' and hienthi>0 order by stt,id desc";

		$sql="select * from $where $limit";

		$tintuc=$db->rawQuery("select * from #_video where hienthi=1 and type=? order by stt asc limit $startpoint,$per_page",array($type));

        $count=$db->rawQueryOne("select COUNT(*) as `numb` from #_video where hienthi=1 and type=?", array($type));
		...
		$title_bar .= $title_cat;
		if(count($tintuc)==0) $title_cat=_noidungdangcapnhat;
	}
