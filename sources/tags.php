<?php  if(!defined('_source')) die("Error");
    ...
    if(!empty($idl)){
        ...
        $product_tags = $db->rawQueryOne("select id,ten_$lang,tenkhongdau_$lang as tenkhongdau,mota_$lang,noidung_$lang,photo,type,cano_$lang from #_tags where id=? and type=? and hienthi=1 order by stt asc",array($idl,$type));
        ...
        if($product_tags){
            ...
            $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau from #_baiviet where hienthi=1 {$subWhere} and type=? $order_by limit $startpoint,$per_page",array('dich-vu'));
