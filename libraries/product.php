<?php
    ...
    class product{
        ...
        public function getPageList(){
            ...
            $items_list = $this->_d->rawQuery("SELECT * from #_baiviet_list where type=? order by stt desc, id desc",array($type));
            ...
            if($id){
                ...
                $items_cat = $this->_d->rawQuery("SELECT * from #_baiviet_cat where type=? and id_list=? order by stt desc, id desc",array($type,$item['id_list']));



                $items_item = $this->_d->rawQuery("SELECT * from #_baiviet_item where type=? and id_cat=? order by stt desc, id desc",array($type,$item['id_cat']));
        ...
        public function saveMan(){
            ...
            if($data['mucgia']){
                $send['mucgia']=str_replace(',','',$data['mucgia']);
            }

            if($data['songuoi']){

                $send['songuoi']=str_replace(',','',$data['songuoi']);

            }

            if($data['sogiuong']){
                $send['sogiuong']=str_replace(',','',$data['sogiuong']);
            }

            if($data['phongtam']){
                $send['phongtam']=str_replace(',','',$data['phongtam']);
            }
                        ...
                        if (isset($_FILES['files'])) {
                            ...
                            for($i=0;$i<count($_FILES['files']['name']);$i++){
                                ...
                                if($_FILES['files']['name'][$i]!=''){
                                    ...
                                    $sendx['ten'] = (isset($_POST['tenhinh'][$i])) ? $_POST['tenhinh'][$i] : "";

                                    $sendx['link'] = (isset($_POST['linkhinh'][$i])) ? $_POST['linkhinh'][$i] : "";
